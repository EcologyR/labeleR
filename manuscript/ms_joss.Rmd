...
This R package consists of eight functions built around a workflow: dataset > R > RMarkdown > LaTeX > PDF. Each function accepts four types of arguments: (1) R-based, including dataframes, paths, and output filenames; (2) fixed, such as the name of the project, event, signer, as well as a path to a logo or image, which remain constant across documents; (3) variable, referring to dataset columns; and (4) mixed, which can be directly entered in the function or linked to dataset columns. Each function is tied to a specific RMarkdown template, allowing a mix of LaTeX language for de desired aestethic outcome and R code to dynamically insert fixed and variable variables specified in the function. The outputs are in pdf format and can be saved locally or automatically emailed to each of the participants for certificate-related functions.
