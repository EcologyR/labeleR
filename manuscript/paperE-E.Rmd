...
subtitle: Julia G. de Aledo, Jimena Mateo-Martín, Francisco Rodríguez-Sánchez, Ignacio Ramos-Gutiérrez*
authors:
...
---
corresponding author: Ignacio Ramos-Gutiérrez/
email: ig.ramosgutierrez@gmail.com
...
# Cover letter
This manuscript is a first submission and authors declare no conflict of interest. The manuscript describes an R package which aims to optimize the creation of different collection labels and documents for scientific events, highly useful in any step of the scientific career. Throughout the manuscript, we discuss previous tools targeting this issue, we state the need for this package, we describe its workflow and we explain the main functions by giving visual examples. The contribution of this package to researchers' daily bases concerns both management, reducing time consuming tasks, and scientific quality, facilitating the sampling of scientific labels for any organism and including features such as the inclusion of pictures or QRs. 
...
Users can also edit and adapt the default `RMarkdown` templates provided by the package for their own purposes. This is useful in case pictures look too big or small, if the size of the text needs to be reduced, the default text needs to be modified, etc. It is then possible to modify these parameters inside the template.
...
Appropriate labelling of samples is a fundamental step of the scientific process (i.e., labelling test tubes in laboratories, storing animal or plant materials or displaying collections in museums or botanical gardens). A user-friendly bulk rendering tool is vital for efficiently producing crafted, uniform labels in a reproducible manner. We present three label types: “herbarium” (most complex), “collection” (most aesthetic) and “tinylabels” (compact and simplified, for small insect collections) (Figure 2). These labels can include QR codes (e.g. links to websites, images, or identification codes) without additional tools, making it easy to quickly access and link to external information. Hereafter there is a short description of each of the functions.

### Herbarium labels

Herbarium labels are fundamental for botanical research since they enable the correct storage and access of information for future research. The `create_herbarium_label` function include multiple variable parameters, tailored to the fields that are classically included in the labels of plant vouchers (species, family, collector, elevation, location, etc.). Note that the `family.column` content will always be capitalized, and the `taxon.column` one in italics, recommended to be used as originally defined, while the rest ca be interchangeable. The QR can include a free text (and therefore remain identical in all labels), or be a column name, which implies that the codes will be rendered with the individual information of each row. The output is a pdf file with four different labels will fit in each of the A4 pdf pages.
...
Collection labels are useful for all type of public or private collections (museums, botanical gardens, living collections, etc.). The `create_collection_label` function counts with five variable parameters, which are not recommended to be too long, along with the possibility of including a QR code (fixed or variable) and an image (logo or picture). Field 1 will be always capitalized, and Field 2 italicized. Any field can be left blank. The user may manually fix the background and text colors with HTML color codes. Eight different labels will fit in each of the A4 pdf pages.
...
This type of labels are a simplified and smaller version of the collection label, suited for lab tubes or insect collections. They include five variable fields, all with the same simple aesthetic, plus the possibility of including a QR code. It is recommended to write short texts for the variable arguments and the QR, as they might become difficult to read. 16 different labels will fit in each of the A4 pdf pages. 
...
Abstract books are useful to compile and distribute all the information of the different contributions of a congress, seminars or reunion. The function `create_abstractbook` result in a single pdf document with multiple pages, each one including one abstract. Each page will include four variable fields (title, author names, affiliations and the abstract texts). The orden of the abstracts mirrors the order of the dataframe rows. If the order of appearance needs to be changed, it is necessary to first arrange the columns in the original dataframe.  The output document can include a table of contents with the titles and page numbers of all abstracts. Additionally, is possible to insert a custom front page that appearing at the beginning of the document.
...
Attendance certificates are meant to certify that different attendants have been present in a specific event. Here, the only variable parameter is the name of the attendees. It allows to include a signature as an image, implying that the signer does not have to sign them individually. This certificate is available both in English and Spanish. 
...
Participation certificates are used to certify that a participant has contributed in a specific event with a talk, a course, a poster, etc. They include multiple variable parameters (such as speaker, affiliation, title, etc.). This document can be rendered in English and in Spanish.
